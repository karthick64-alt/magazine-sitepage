const magazineData = {
  ...
  categoryDetails: {
    Technology: {
      description: "Explore the latest innovations in AI, computing, and digital transformation shaping our future.",
      icon: "fas fa-microchip"
    },
    Lifestyle: {
      description: "Discover stories about urban living, community building, and mindful design that enhance daily life.",
      icon: "fas fa-heart"
    },
    Travel: {
      description: "Journey through regenerative travel, sustainable tourism, and cultural exploration around the globe.",
      icon: "fas fa-map-marked-alt"
    },
    Business: {
      description: "Insights into modern entrepreneurship, remote work, and transparent business practices.",
      icon: "fas fa-briefcase"
    },
    Design: {
      description: "Architecture, interfaces, and spaces that blend aesthetics with functionality and mindfulness.",
      icon: "fas fa-palette"
    },
    "Health & Fitness": {
      description: "Wellness, fitness, and health innovations that promote holistic living and vitality.",
      icon: "fas fa-dumbbell"
    },
    Entertainment: {
      description: "Culture, media, and entertainment trends shaping how we consume and create content.",
      icon: "fas fa-film"
    }
  },
  categoryPosts: {
  ...
  header: `
    <nav class="navbar navbar-expand-lg bg-body border-bottom py-3 fixed-top">
      <div class="container">
        ...
        <button class="navbar-toggler" type="button" data-bs-toggle="offcanvas" data-bs-target="#primaryNav" aria-controls="primaryNav" aria-label="Toggle navigation" aria-expanded="false">
...
function renderCategoryPage() {
  ...
  const statsEl = document.querySelector("[data-category-stats]");
  ...
  // Update description and stats if category is selected
  if (name && magazineData.categoryDetails[name]) {
    const details = magazineData.categoryDetails[name];
    const count = list.length;
    descriptionEl.innerHTML = `<i class="${details.icon} me-2 text-primary"></i>${details.description}`;
    statsEl.innerHTML = `
      <div class="d-flex justify-content-center align-items-center gap-4 flex-wrap">
        <div class="text-center">
          <div class="h4 mb-0 text-primary">${count}</div>
          <small class="text-muted">${count === 1 ? 'Article' : 'Articles'}</small>
        </div>
        <div class="vr d-none d-md-block"></div>
        <div class="text-center">
          <div class="h4 mb-0 text-primary">${magazineData.categories.length}</div>
          <small class="text-muted">Categories</small>
        </div>
      </div>
    `;
  } else {
    const totalArticles = magazineData.latest.length + Object.values(magazineData.categoryPosts).flat().length;
    descriptionEl.textContent = "Filter by category to explore our latest coverage across technology, lifestyle, travel, business, and more.";
    statsEl.innerHTML = `
      <div class="d-flex justify-content-center align-items-center gap-4 flex-wrap">
        <div class="text-center">
          <div class="h4 mb-0 text-primary">${totalArticles}</div>
  ...
  const filterButtons = document.querySelectorAll(".card-body .d-grid a.btn");
  filterButtons.forEach((btn) => {
    const href = btn.getAttribute("href");
    if (!name && href === "category.html") {
      btn.classList.remove("btn-outline-secondary");
      btn.classList.add("btn-primary");
    } else if (name && href.includes(`name=${encodeURIComponent(name)}`)) {
      btn.classList.remove("btn-outline-secondary");
      btn.classList.add("btn-primary");
    } else {
